...
namespace AssetStudio
{
    public static class AIVersionManager
    {
        ...
        private const string VersionIndexName = "版本索引.json";
        ...
        private static async Task<string> DownloadString(string url, TimeSpan timeout)
        {
            ...
            if (CreateUri(url, out var uri))
            {
                ...
                {
                    Logger.Warning($"下载超时{Path.GetFileName(url)}, {ex.Message}");
                }
                catch (Exception ex)
                {
                    Logger.Warning($"下载失败{Path.GetFileName(url)}, {ex.Message}");
                }
            }
            return content;
        }
        ...
        private static async Task<string> GetLatestCommit(string path)
        {
            ...
            {
                Logger.Warning($"无法解析最新提交{Path.GetFileName(path)}");
            }
            return commit;
        }
