    ...
    public class AssetsManager
    {
        ...
        private void LoadMhyFile(FileReader reader, string originalPath = null, long originalOffset = 0, bool log = true)
        {
            ...
            try
            {
                ...
                Logger.Verbose($"米哈游总尺寸: {mhyFile.m_Header.size:X8}");
        ...
        private void LoadBlbFile(FileReader reader, string originalPath = null, long originalOffset = 0L, bool log = true)
        {
            if (log)
            {
                Logger.Info("加载中" + reader.FullPath);
            ...
            try
            {
                ...
                {
                    var dummyPath = Path.Combine(Path.GetDirectoryName(reader.FullPath), file.fileName);
                    // 按照 LoadBlbFile 的逻辑，明确传入第三个参数false
                    var cabReader = new FileReader(dummyPath, file.stream, false);
                    if (cabReader.FileType == FileType.AssetsFile)
                    {
