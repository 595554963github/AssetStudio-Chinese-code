...
namespace AssetStudio.GUI
{
    ...
    internal static class Studio
    {
        ...
        {
            StatusStripUpdate($"解压{reader.FileName} ...");
            try
            {
            ...
            {
                Logger.Error($"游戏类型不匹配,预期为{nameof(Mr0k)}然而是{Game.Name} ({Game.GetType().Name}) !!");
            }
            return 0;
        }
        ...
        private static int ExtractMhyFile(FileReader reader, string savePath)
        {
            ...
            {
                Logger.Error($"游戏类型不匹配,预期为{nameof(Mhy)}然而是{Game.Name} ({Game.GetType().Name}) !!");
            }
            return 0;
        }
        ...
        public static (string, List<TreeNode>) BuildAssetData()
        {
            ...
            foreach((var pptr, var name) in mihoyoBinDataNames)
            {
                ...
                if (pptr.TryGet<MiHoYoBinData>(out var obj))
                {
                    ...
                    else assetItem.Text = $"二进制文件{assetItem.m_PathID}";
                }
            }
            if (!SkipContainer)
            {
        ...
        public static Task ExportAssets(string savePath, List<AssetItem> toExportAssets, ExportType exportType, bool openAfterExport)
        {
            return Task.Run(() =>
            {
                ...
                foreach (var asset in toExportAssets)
                {
                    ...
                    {
                        Logger.Error($"导出{asset.Type}:{asset.Text} error\r\n{ex.Message}\r\n{ex.StackTrace}");
                    }

                    Progress.Report(++i, toExportCount);
                }

                var statusText = exportedCount == 0 ? "什么都没导出." : $"导出{exportedCount}资源完成.";

                if (toExportCount > exportedCount)
                {
                    statusText += $" {toExportCount - exportedCount}资源跳过(不可提取或文件已存在)";
                }
            ...
            return Task.Run(() =>
            {
                ...
                switch (exportListType)
                {
                    case ExportListType.XML:
                        var filename = Path.Combine(savePath, "资源.xml");
                        ...
                        using (XmlWriter writer = XmlWriter.Create(filename, settings))
                        {
                            ...
                            writer.WriteAttributeString("创建dat", DateTime.UtcNow.ToString("s"));
                            foreach (var asset in toExportAssets)
                            {
                                writer.WriteStartElement("资源");
                                ...
                                writer.WriteElementString("路径ID", asset.m_PathID.ToString());
                                writer.WriteElementString("源", asset.SourceFile.fullName);
                                writer.WriteElementString("大小", asset.FullSize.ToString());
                                writer.WriteEndElement();
                            }
                            writer.WriteEndElement();
                ...
                var statusText = $"导出清单中{toExportAssets.Count()}资源目标完成.";
            ...
            return Task.Run(() =>
            {
                ...
                foreach (TreeNode node in exportNodes)
                {
                    ...
                    foreach (GameObjectTreeNode j in node.Nodes)
                    {
                        ...
                        {
                            Logger.Error($"导出游戏对象:{j.Text} error\r\n{ex.Message}\r\n{ex.StackTrace}");
                        }

                        Progress.Report(++k, count);
                        StatusStripUpdate($"到处完成{filename}.fbx");
                    }
                }
        ...
        public static Task ExportAnimatorWithAnimationClip(AssetItem animator, List<AssetItem> animationList, string exportPath)
        {
            return Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    Progress.Report(1, 1);
                    StatusStripUpdate($"导出完成{animator.Text}");
                }
                catch (Exception ex)
                {
                    Logger.Error($"导出动画:{animator.Text} error\r\n{ex.Message}\r\n{ex.StackTrace}");
                ...
                if (gameObjects.Count > 0)
                {
                    ...
                    foreach (var gameObject in gameObjects)
                    {
                        ...
                        try
                        {
                            ...
                            StatusStripUpdate($"导出完成{gameObject.m_Name}");
                        }
                        catch (Exception ex)
                        {
                ...
                foreach (var node in nodes)
                {
                    ...
                    if (gameObjects.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            StatusStripUpdate($"导出错误{name}");
                        }
                        catch (Exception ex)
                        {
                            Logger.Error($"导出错误:{name} 错误\r\n{ex.Message}\r\n{ex.StackTrace}");
                            StatusStripUpdate("导出错误");
                        }
                    }
                    else
                    {
                        StatusStripUpdate("选择导出的空节点.");
                    }
                }
                if (Properties.Settings.Default.openAfterExport)
                {
